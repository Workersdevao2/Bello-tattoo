...
(function () {
  ...
  // ---------- Video autoplay (Safari Mac / iOS safe) ----------
  // Safari needs: muted attribute + property, defaultMuted, volume 0,
  // playsinline, and an explicit play(). Autoplay can still fail until
  // a user gesture — we unlock on first interaction.
  function prepareMuted(video) {
    if (!video) return;
    video.muted = true;
  ...
  function forceVideoPlay(video) {
    ...
    if (video.readyState >= 2) {
      ...
      // Safari sometimes never fires those if preload is delayed
      setTimeout(function () {
  ...
  function playAllAutoplayVideos() {
    document.querySelectorAll("video[autoplay], .hero-video.is-active").forEach(forceVideoPlay);
  }

  playAllAutoplayVideos();

  // Safari pauses background tabs — resume when visible again
  ...
  // Unlock autoplay after first user gesture (Safari Mac policy)
  var unlocked = false;
  function unlockVideos() {
    if (unlocked) return;
    unlocked = true;
    playAllAutoplayVideos();
    document.removeEventListener("click", unlockVideos, true);
    document.removeEventListener("touchstart", unlockVideos, true);
    document.removeEventListener("keydown", unlockVideos, true);
  }
  document.addEventListener("click", unlockVideos, true);
  document.addEventListener("touchstart", unlockVideos, true);
  document.addEventListener("keydown", unlockVideos, true);
  ...
  if (heroStack) {
    ...
    function playVideo(v) {
      ...
      const tryPlay = function () {
        prepareMuted(v);
        // Avoid seeking if already near start — Safari can stall on currentTime=0
        try {
          if (v.readyState >= 1 && v.currentTime > 0.15) {
            v.currentTime = 0;
          }
        } catch (e) {}
        const p = v.play();
      ...
      if (v.readyState >= 2) {
        ...
        setTimeout(function () {
          if (v.paused && videos[current] === v) tryPlay();
        }, 700);
      }
    }
    ...
    function scheduleNext(fromIndex) {
      ...
      // Warm up next clip without calling load() (load() breaks Safari buffer)
