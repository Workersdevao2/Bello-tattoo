...
(function () {
  ...
  if (heroStack) {
    ...
    function preloadVideo(v) {
      ...
      // Only force load if almost nothing is buffered yet
      if (v.readyState < 2) {
        try {
          v.load();
        } catch (e) {}
    ...
    function playVideo(v) {
      ...
      const tryPlay = () => {
        ...
        if (p && typeof p.catch === "function") {
          p.catch(() => {
            // Safari / iOS often needs a second attempt
            setTimeout(() => {
              v.muted = true;
              v.play().catch(() => {});
            }, 250);
      ...
      // Seek to start only when we have data
      const seekAndPlay = () => {
        try {
      ...
      if (v.readyState >= 2) {
        ...
        // Safety: try anyway after a short wait
        setTimeout(() => {
          if (v.paused && videos[current] === v) seekAndPlay();
        }, 600);
      }
    }
    ...
    function scheduleNext(fromIndex) {
      ...
      // Preload the next clip while current plays
    ...
    function showHero(index) {
      ...
      // Allow next switch after transition settles
      setTimeout(() => {
        switching = false;
      }, 400);
    }
    ...
    videos.forEach((v, i) => {
      ...
      // Soft-error recovery: if a video errors, skip to next
    ...
    // Kick off: preload all lightly, then start first
    videos.forEach((v, i) => {
      if (i === 0) {
        v.preload = "auto";
      } else {
        v.preload = "auto"; // better than metadata so rotation doesn't freeze
      }
    });
    showHero(0);
  }
